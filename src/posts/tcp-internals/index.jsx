...
nstat -az | grep -E 'TcpRetransSegs|TcpOutSegs|TcpExtTCPLostRetransmit|ListenDrops'`} />
      ...
      <section>
        ...
        <p>
          If reliable delivery at this layer was interesting, the same problem reappears one layer up with different trade-offs: <Link to="/kafka-internals">Kafka beyond the basics</Link> covers ISR, the high watermark and what <code>acks=all</code> actually buys you — durability built on top of the delivery guarantee described here. <Link to="/postgres-internals">PostgreSQL storage internals</Link> takes it to disk, where the WAL is doing for a database what sequence numbers and ACKs do for a byte stream. <Link to="/monitoring-agent-internals">The anatomy of a lightweight monitoring agent's performance</Link> applies the same one-layer-deeper habit to a different target — the tool watching all of these systems, debugged through <code>/proc</code> instead of a wire capture. For distributed agreement on top of all of it, there are interactive visualizers for <a href="https://raft.string-wise.com" target="_blank" rel="noopener noreferrer">Raft consensus</a> and <a href="https://hash.string-wise.com" target="_blank" rel="noopener noreferrer">consistent hashing</a>.
        </p>
      </section>
    </article>
  )
